...
class CartController extends GetxController {
  ...
  void increaseQuantity(int index) async {
    if (index >= 0 && index < productSnapshot.length) {
      DocumentReference docRef = productSnapshot[index].reference;
  ...
  void decreaseQuantity(int index) async {
    if (index >= 0 && index < productSnapshot.length && productSnapshot[index]['qty'] > 1) {
      DocumentReference docRef = productSnapshot[index].reference;
      DocumentSnapshot docSnapshot = await docRef.get();
  ...
  updateTotalPrice() {
    ...
    for (var item in productSnapshot) {
      totalP.value = (totalP.value + (item['tprice'] * item['qty'])).toInt();
    }
  }
